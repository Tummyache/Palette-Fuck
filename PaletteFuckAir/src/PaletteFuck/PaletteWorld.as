	...
	public class PaletteWorld extends World 
	{
		...
		override public function update():void 
		{
			...
			if (Input.pressed(Key.LEFT))
			{
				...
				var straightDistance = Math.abs(mSelectedBlock.mColor.Hue-mShadowSlider.mShadowColor);
				var leftDistance = Math.abs(mSelectedBlock.mColor.Hue - (mShadowSlider.mShadowColor-360));
				var rightDistance = Math.abs(mSelectedBlock.mColor.Hue - (360 + mShadowSlider.mShadowColor));
				if (straightDistance < leftDistance)
				{
					if (straightDistance < rightDistance)
					{
						direction = FP.sign(mShadowSlider.mShadowColor-mSelectedBlock.mColor.Hue);
					}
					else
					{
						direction = 1;
				...
				shadowBlock.mColor.Hue = mSelectedBlock.mColor.Hue + direction * 14;
			...
			else if (Input.pressed(Key.RIGHT))
			{
				...
				var direction:int = 0;
				var straightDistance = Math.abs(mSelectedBlock.mColor.Hue-mShadowSlider.mHighlightColor);
				var leftDistance = Math.abs(mSelectedBlock.mColor.Hue - (mShadowSlider.mHighlightColor-360));
				var rightDistance = Math.abs(mSelectedBlock.mColor.Hue - (360 + mShadowSlider.mHighlightColor));
				if (straightDistance < leftDistance)
				{
					if (straightDistance < rightDistance)
					{
						direction = FP.sign(mShadowSlider.mHighlightColor-mSelectedBlock.mColor.Hue);
					}
					else
					{
						direction = 1;
				...
				lightBlock.mColor.Hue = mSelectedBlock.mColor.Hue + direction * 14;
